...
Book.all.each do |i|
  if i.category.name == "Romance"
    romance_list << [i.name, i.author.name]
  elsif i.category.name == "Thriller"
    thriller_list << [i.name, i.author.name]
  elsif i.category.name == "Classics"
    classics_list << [i.name, i.author.name]
  end
end
...
while runner == 0
  ...
  Book.all.each do |i|
    genre_list << i.category.name
  end

  puts "What type of book would you like to read?"
  uniq_list = genre_list.uniq!
  uniq_list.each_with_index do |sample, index|
    puts "[#{index + 1}] #{sample}"
  end
